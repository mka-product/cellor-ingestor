...
class FileReviewRepository:
    ...
    def save_layer(self, layer: AnnotationLayer) -> AnnotationLayer:
        payload = self._read()
        bucket = payload["slides"].setdefault(layer.slide_id.value, {"layers": [], "annotations": [], "comments": []})
        serialized = {
    ...
    def delete_layer(self, slide_id: SlideId, layer_id: AnnotationLayerId) -> None:
        payload = self._read()
        bucket = payload["slides"].setdefault(slide_id.value, {"layers": [], "annotations": [], "comments": []})
    ...
    def save_annotation(self, annotation: AnnotationFeature) -> AnnotationFeature:
        ...
        bucket = payload["slides"].setdefault(
            annotation.slide_id.value, {"layers": [], "annotations": [], "comments": []}
        )
        existing = next(
    ...
    def save_comment(self, comment: AnnotationComment) -> AnnotationComment:
        payload = self._read()
        bucket = payload["slides"].setdefault(comment.slide_id.value, {"layers": [], "annotations": [], "comments": []})
    ...
    def _slide_bucket(self, slide_id: SlideId) -> dict[str, Any]:
        payload = self._read()
        return payload["slides"].setdefault(slide_id.value, {"layers": [], "annotations": [], "comments": []})
